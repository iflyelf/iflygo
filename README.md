...
```
              ...
              ▼                                                         ▼
    ┌───────────────────┐                                   ┌───────────────────┐
    │  Client A (客户端) │                                   │  Client B (客户端) │
    │  am_lighthouse:    │  ②③ UDP 打洞 (NAT 穿透)            │  am_lighthouse:    │
    │       false        │◄═════════════════════════════════►│       false        │
    │  10.88.0.100       │      建立 P2P 加密直连隧道          │  10.88.0.101       │
    │  动态端口 (port: 0) │      (数据不经过 lighthouse)        │  动态端口 (port: 0) │
    └─────────┬─────────┘                                   └─────────┬─────────┘
              │                                                       │
              │            ④ 当 P2P 直连失败时(对称NAT等)              │
              │               流量经由 Relay 中转                      │
